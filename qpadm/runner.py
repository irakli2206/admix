...
def _exit_hint(code: int) -> str:
    if code < 0:
        ...
        if sig == 11:
            return " (SIGSEGV — qpAdm crashed; try smaller runs, allsnps:NO, or different qpAdm build)"
    return ""
    ...
    try:
        ...
        if not os.path.isfile(par_path):
            raise FileNotFoundError(
                f"Parameter file not found after extract: {par_name!r}"
            )
        ...
        if materialized:
            logger.info("qpAdm job %s: %s", job_id, "; ".join(materialized))
